...
struct Node {
	...
	__always_inline Access _hasPermission(const uint64_t hashes[], const int sz) const {
		...
		for (int i = 0; i < sz; ++i) {
			...
			if (it == nodes.end()) {// requested node not found - return wildcard status
				found = false;
				break;
			}

			start = &it->second;
			if (start->wildcard) {		 // skip nested nodes - return state of root?
	...
	__always_inline void deletePerm(const uint64_t hashes[], const int sz) {
		...
		Node* t_node = this;
		for (int i = 0; i < sz; ++i) {
			if (t_node->nodes.contains(hashes[i]))
				t_node = &t_node->nodes.at(hashes[i]);
			else
				return;
		}
		if (!lwildcard) {
			auto it = t_node->nodes.find(hashes[sz - 1]);
			if (it != t_node->nodes.end())
				t_node->nodes.erase(it);
		} else
			t_node->nodes.clear();
	}
	__always_inline void addPerm(const plg::string& perm) {
		...
		for (auto s: spl) {
			...
			if (ss == "*")
				continue;
			if (uint64_t hash = XXH3_64bits(ss.data(), ss.size()); this->nodes.contains(hash))
				node = &this->nodes.at(hash);
			else
				node = &(node->nodes.at(hash) = Node(false, false, plg::string(ss), std::unordered_map<uint64_t, Node>()));
		}
		node->state = lstate;
...
inline void forceRehash(std::unordered_map<uint64_t, Node>& nodes) {
	nodes.rehash(0);
	for (std::pair<const uint64_t, Node>& n: nodes)
		forceRehash(n.second.nodes);
}
...
inline Node loadNode(const plg::vector<plg::string>& perms) {
	...
	for (const auto& perm: perms) {
		...
		if (iperm->empty())// empty lines?
			continue;
		if (iperm->at(0) == '*') {// skip situation with perm "*"
		...
		for (auto s: spl) {
			...
			if (ss == "*")
				break;
			if (uint64_t hash = XXH3_64bits(ss.data(), ss.size()); node->nodes.contains(hash))
				node = &node->nodes.at(hash);
			else {
				node = &(node->nodes[hash] = Node(false, false, plg::string(ss), std::unordered_map<uint64_t, Node>()));
			}
		}
